...
#Code starts
source("/home/CAM/jcotney/TOOLS/FourCSeq/fourcseq_revised_code_150316.R")
library(FourCSeq)

#description of the fourC viewpoint
metaData <- SimpleList(projectPath = projectname,
...
#find viewpoint
#addViewpointFrags, default minSize = 20, filter = TRUE, save = FALSE
findViewpointFragments(fc)
fc <- addViewpointFrags(fc)

#Count the reads for fragments #need to trim 4 bp for the restriction site
#edit overlaptype to "start" to go back to the default overlap
fc<-countFragmentOverlaps_edi(fc, trim=4, minMapq=-1, overlaptype="any") #need to trim 4bp for GATC or CATG
...
#edit maxDist to limit the maximum distance of the fragment to the viewpoint (e.g. 1000000)
fcf <- getZScores_edi(fc, maxDist=NULL, minCount = 40) 
...
#write the results
results<-cbind(as.vector((rowRanges(fcf)@seqnames)),start(rowRanges(fcf)@ranges),end(rowRanges(fcf)@ranges),assay(fcf,"counts"),assay(fcf,"zScore"),assay(fcf,"pValue"),
